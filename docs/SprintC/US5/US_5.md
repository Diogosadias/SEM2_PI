...
**Q1:** During the current sprint, how should we allow the barcodes to be printed. After generating them via the API, should we save the barcode images to the disk?

**A1**: Each generated barcode should be saved in a folder as a jpeg file.

**Q2**: It was answered in one of the previous questions that the sample only has one attribute, the barcode, however the description of the project also mentions a date of collection of the samples, is this date supposed to be an attribute of the test and not of the sample itself?

**A2**: Yes. The system should record the date (DD/MM/YYYY) and time (HH:MM) when the sample is collect made. The date and time are automatically generated by the system when the barcode is issued.

**Q3:** If there were multiple samples for a single test, would there be only one collection date for all of them?

**A3:** Only one collection date and time for a test.

**Q4:** Why is there the specification for only the tests with no samples?

**A4:** The Medical Lab Technician selects a tests and the number of barcodes he/she wants. Then the system generates the requested barcodes.
After generating the barcodes, the select test is no more available in the system for generating barcodes.

**Q5:** Shouldn't there be a way for technicians to add more samples to an existing test?
**A5:** No.

**Q6:** Should there be a validation of the number of samples?

**A6:** No.

**Q7:** After the barcodes are generated. The concept-reality link would be, for example to stick each barcode into each sample?

**A7:** Yes. Each generated barcode should be saved in a folder as a jpeg file.
...
**AC2:** When the User finishes the sample registering task, the test can no longer be registered with more samples.

**AC3:** A test can have more than one sample

**AC4:** A barcode should be generated as a jpeg.
...
Output data:
+ A new Sample is registered on the test
+ Sample's Barcode;
...
| Step 5: inputs sample number 		 |		...saving input data?					 |     TestStore        |         IE: knows Sample Object data                     |
