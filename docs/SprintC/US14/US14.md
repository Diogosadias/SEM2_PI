...
| Step 2  : Shows list of Tests (Completed)	and Request Selection of Test	 |...Showing the tests?|Test -> TestStore| Based on the IE Standard (Test Used to Store the list of Tests) on the MD,the responsibility is to the Test. By the application of HC+LC in Test the responsibility is delegated to TestStore.
| Step 3  : Select the test to write the report		 |...Selecting the Test?|TestStore| IE: Has the list of all tests.
| Step 4  : shows the Test information and requests the Data (Diagnosis)  |...Showing the information?|TestStore|IE: Knows the information to be visualized.
...
| Step 6  : shows the Test information and requests Confirmation  |...validating the data according to AC?|Report|IE: Know its own creation rules.
| Step 7  : Confirms data		 |...saving the Report written?|SpecialistDoctor -> ReportStore |   Based on the Creator standard (SpecialistDoctor used to store the list of all Reports ) on the MD, the responsibility is attributed to the SpecialistDoctor. By application of HC + LC in Company the responsibility is delegated to ReportStore.           
